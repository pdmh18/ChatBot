...
def predict_task_risk(req: TaskRiskRequest):
    """Dự báo xác suất 1 task bị trễ hạn."""
    try:
...
def match_staff(req: StaffMatchRequest):
    """Xếp hạng nhân sự phù hợp nhất cho 1 task."""
    return rank_candidates(req.ky_nang_yeu_cau, req.do_phuc_tap, req.top_n)


@router.get("/analyze-bottleneck", response_model=list[BottleneckResult])
def analyze_bottleneck(top_n: int = 5):
    """Phân tích đồ thị phụ thuộc công việc, tìm điểm nghẽn dự án."""
    G = build_graph()
